          ...
          const renderRanges = () => {
            ...
            ranges.forEach((r, ri) => {
              ...
              f.onchange = () => { 
                ...
                // Copy array
                const newArr = [...(this._config.color_ranges?.[eid] || [])];
                // Copy object and update
                if(newArr[ri]) newArr[ri] = { ...newArr[ri], from: this._fromDisplayTemp(v) };
                this._config.color_ranges = { ...(this._config.color_ranges||{}), [eid]: newArr }; 
                repaint(); 
              };
              t.onchange = () => { 
                let v=parseFloat(t.value); if (isNaN(v)) v=minDisp; v=Math.max(minDisp, Math.min(maxDisp, v)); t.value=String(v); 
                const newArr = [...(this._config.color_ranges?.[eid] || [])];
                if(newArr[ri]) newArr[ri] = { ...newArr[ri], to: this._fromDisplayTemp(v) };
                this._config.color_ranges = { ...(this._config.color_ranges||{}), [eid]: newArr }; 
                repaint(); 
              };
              c.onchange = () => { 
                const newArr = [...(this._config.color_ranges?.[eid] || [])];
                if(newArr[ri]) newArr[ri] = { ...newArr[ri], color: c.value };
                this._config.color_ranges = { ...(this._config.color_ranges||{}), [eid]: newArr }; 
                repaint(); 
              };
              rm.onclick = () => { 
